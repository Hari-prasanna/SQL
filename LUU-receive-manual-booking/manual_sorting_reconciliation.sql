...
WITH
    ...
    normal_goods_t1 AS (
        ...
        WHERE hv.TPARTNR = 520
            ...
            -- Performance optimization: Filter dates early inside the CTE
    ...
    -- =========================================================================
    -- PART 2: DUMMY GOODS (Advanced Logic with ROW_NUMBER Fix)
    -- =========================================================================
    dummy_goods_t1 AS (
        SELECT /*+ MATERIALIZE */
            hv.LOCAL_TRANSACTION_ID,
            ...
            hv.CUST_DATA,
            JSON_VALUE(hv.CUST_DATA, '$.REFERENCENUMBER_LHM') AS Reference_LHM,
            -- TECHNICAL FIX: Add Row Number to prevent duplicates in the dummy join
            -- This assigns a unique index (1, 2, 3...) to each step in the transaction
    ...
    dummy_goods_t2 AS (
        SELECT /*+ MATERIALIZE */
            ...
            hv.CUST_DATA,
            hv.ARTNR,
            -- TECHNICAL FIX: Add Row Number to match exact sequence of t1
            ROW_NUMBER() OVER (PARTITION BY hv.LOCAL_TRANSACTION_ID ORDER BY hv."SEQUENCE" ASC) as rn
        FROM HISTORIE_V hv
...
SELECT
    ...
    -- Decode JSON Codes to Human Readable Text
    ...
    -- Logic to determine Distribution Channel
    CASE
    ...
    -- Dynamic SKU Extraction based on Goods Type
    CASE
