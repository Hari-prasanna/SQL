...
WITH
    ...
    dummy_goods_t1 AS (
        ...
        FROM
            TRANSACTION_HISTORY_V hv
        WHERE
            hv.TPARTNR = 2 -- DUMMY_GOODS_SOURCE_A (illustrative placeholder, not a real partner code)
            AND hv.MENGE < 0
            AND hv.LAGBEZ IN ('Overstock', 'SZROV')
            AND hv.ZIEL LIKE 'OV%'
    ),
    ...
    dummy_goods_t2 AS (
        ...
        FROM
            TRANSACTION_HISTORY_V hv
        WHERE
            hv.TPARTNR = 3 -- DUMMY_GOODS_SOURCE_B (illustrative placeholder, not a real partner code)
            AND hv.LAGBEZ IN ('Overstock', 'SZROV')
            AND hv.MENGE = 1
            AND hv.ZIEL LIKE 'OV%'
    ),
