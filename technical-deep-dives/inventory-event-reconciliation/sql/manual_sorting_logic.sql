 ...
 * production schema. Partner codes below are illustrative placeholders, not real
 * production values. See ../../../docs/sanitization-policy.md.
 */
...
WITH
    ...
    normal_goods_t1 AS (
        ...
        FROM TRANSACTION_HISTORY_V hv
        WHERE hv.TPARTNR = 1 -- NORMAL_GOODS_SOURCE (illustrative placeholder, not a real partner code)
    ...
    normal_goods_t2 AS (
        ...
        FROM TRANSACTION_HISTORY_V hv
        WHERE hv.TPARTNR = 1 -- NORMAL_GOODS_SOURCE
            AND hv.ZIEL IN ('IN_SZR_OV', 'SZR_OV')
            AND hv.MENGE = 1
            -- Performance: Filter Date here
            AND (:start_datetime IS NULL OR hv.CREATED BETWEEN TO_DATE(:start_datetime, 'DD.MM.YYYY HH24:MI:SS') 
    ...
    dummy_goods_t1 AS (
        ...
        FROM TRANSACTION_HISTORY_V hv
        WHERE hv.TPARTNR = 2 AND hv.TYP_ID = 120 -- DUMMY_GOODS_SOURCE_A (illustrative placeholder, not a real partner code)
            AND hv.MENGE < 0 AND hv.MENGE IS NOT NULL
    ...
    dummy_goods_t2 AS (
        SELECT /*+ MATERIALIZE */
            ...
            ROW_NUMBER() OVER (PARTITION BY hv.LOCAL_TRANSACTION_ID ORDER BY hv."SEQUENCE" ASC) as rn
        FROM TRANSACTION_HISTORY_V hv
        WHERE hv.TPARTNR = 3 -- DUMMY_GOODS_SOURCE_B (illustrative placeholder, not a real partner code)
            AND hv.ZIEL IN ('IN_SZR_OV', 'SZR_OV')
            AND hv.MENGE > 0
            -- Performance: Filter Date here
            AND (:start_datetime IS NULL OR hv.CREATED BETWEEN TO_DATE(:start_datetime, 'DD.MM.YYYY HH24:MI:SS') 
...
SELECT
    ...
    DECODE(JSON_VALUE(ag.CUST_DATA, '$.SOURCEID_SEKTOR'),
        '1',  'Primary Channel', '10', 'OSR', '11', 'OSR (OV)', 'Partner Channel'
    ) AS Source_Channel,
